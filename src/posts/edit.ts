...
module.exports = function (Posts: PostObjectNew) {
    ...
    function getEditPostData(data, topicData, postData) {
        const editPostData: edit = {
            content: data.content,
            editor: data.uid,
            edited: topicData.scheduled ? (postData.edited || postData.timestamp) + 1 : Date.now(),
