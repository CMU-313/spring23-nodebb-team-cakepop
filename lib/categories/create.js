...
module.exports = function (Categories) {
    Categories.create = async function (data) {
        ...
        await db.sortedSetAddBulk([['categories:cid', category.order, category.cid], [`cid:${parentCid}:children`, category.order, category.cid], ['categories:name', 0, `${data.name.slice(0, 200).toLowerCase()}:${category.cid}`]]);
